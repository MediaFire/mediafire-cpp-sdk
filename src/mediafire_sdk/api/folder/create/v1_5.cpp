...
namespace v1_5 = mf::api::folder::create::v1_5;


namespace {
std::string AsString(const v1_5::OnNameCollision & value)
{
    if (value == v1_5::OnNameCollision::Rename)
        return "keep";
    if (value == v1_5::OnNameCollision::ReturnExistingFolderkey)
        return "replace";
...
const std::string api_path("/api/1.5/folder/create");
...
public:
    ...
    boost::optional<std::string> parent_path_;
...
mf::http::SharedBuffer::Pointer Impl::GetPostData()
{
    ...
    if (parent_path_)
        parts["parent_path"] = *parent_path_;
    if (mtime_)
        parts["mtime"] = AsString(*mtime_);
    if (rename_on_name_collision_)
        parts["action_on_duplicate"] = AsString(*rename_on_name_collision_);
...
void Request::SetParentPath(std::string parent_path)
{
    impl_->parent_path_ = parent_path;
}
